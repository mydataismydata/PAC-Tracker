...
- **People are subjects too.** Chair and treasurer sit under a committee's name with
  how many committees each is named on; clicking one opens *that person* in the same
  panel a committee gets — money in, money out, every transaction, funding origins —
  computed over every committee naming them. For the largest network in the live data
  that is **107 committees, $95.9M received from 2,490 counterparties, 54% of it traced
  to originating sources**. Money the group moved between its own committees is
  detected and reported separately (**$33.3M** of that $95.9M): it is real, but it
  neither entered nor left the network, and a total that includes it overstates what
  was raised.
...
The count is therefore shown wherever a name is: the chair/treasurer line under
a committee's name carries `×104`, greyed once it is large enough to read as a
service provider. Two contrasting cases in the live data:
...
**Misspellings split a person in two, and only a human should merge them.**
`officerKey` folds middle names, initials and punctuation away, so "William
Stafford Jones" and "William S. Jones" key together. It cannot cross a
misspelt name, and the state's list contains several: "Williams S Jones" and
"Wiliam S Jones" keyed apart from the same man, hiding seven committees from a
network of a hundred. Fuzzy-matching officer names automatically is not the fix
— two people genuinely called J. Smith are common, and a wrong merge asserts
that a *named individual* runs committees they have nothing to do with. So
corrections live as enumerated rows in `officer_aliases`, applied at ingest and
