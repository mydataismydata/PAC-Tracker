...
function highlightRoute(cy: Core, chain: string[], keepLit: string[]): boolean {
  ...
  // Paperwork, not money: also keep lit any registration hop tying a tile that
  // is staying on screen — a money-route tile, or the seed/trail kept readable —
  // to an officer hub (the treasurer, chair or agent behind those committees),
  // plus that hub. The officer usually hangs off the entity the trace was run
  // on, which is the seed rather than a node on the money route, so anchoring
  // only to the route missed it. A dashed registration edge records no payment,
  // so lighting it beside the route claims no money the route did not. Only the
  // hub and the single hop reaching it light; the hub's own links to its other
  // committees stay dimmed, so the focus does not sprawl.
  const anchors = route.nodes().union(lit);
  const regHops = anchors.connectedEdges('edge[kind = "registration"]');
